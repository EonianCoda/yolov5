...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    optimizer.add_param_group({'params': bias})  # add projection network bias
    optimizer.add_param_group({'params': weight, 'weight_decay': hyp['weight_decay']})  # add projection network weight
    del bias, weight
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni <= nw:
                ...
                for j, x in enumerate(optimizer.param_groups):
                    ...
                    x['lr'] = np.interp(ni, xi, [hyp['warmup_bias_lr'] if j == 2 or j == 3 else 0.0, x['initial_lr'] * lf(epoch)])
                    if 'momentum' in x:
